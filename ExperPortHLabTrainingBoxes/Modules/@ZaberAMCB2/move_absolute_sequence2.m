function move_absolute_sequence2(z, seq, motorNumSeq)
%
...
if ~isa(motorNumSeq,'numeric')
    error('Argument motorNumSeq must be numeric')
end
 
if length(motorNumSeq)~=length(seq)
    error('motorNumSeq and seq must be the same size')
end
seq2 = seq;
randSeq = (randperm(length(seq)));
for i=1:length(seq)
    seq = seq2(randSeq(i));
    unit = motorNumSeq(randSeq(i));
